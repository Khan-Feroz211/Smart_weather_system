...
class AlertStore:
    ...
    The production implementation uses Supabase PostgreSQL, but the class also
    supports a local SQLite fallback. This allows the app to keep working in
    local/demo mode and lets tests inject a temporary database via ``db_path``.
    """

    def __init__(self, db_path: Optional[str] = None):
        self.db_path = db_path
        self._fallback_db_path = os.path.join(os.getcwd(), "multi_hazard_alerts.db")

    def _sqlite_db_path(self) -> str:
        return self.db_path or self._fallback_db_path

    def _ensure_sqlite_schema(self) -> None:
        target_db = self._sqlite_db_path()
        import sqlite3
    ...
    def store_alert(self, alert: Dict[str, Any]) -> int:
        ...
        try:
            import db
            if self.db_path is None and db.is_configured():
                with db.connect() as conn:
                    ...
                    """, (
                        ...
                        float(alert["hazard_probabilities"].get("normal", 0.0)),
                        alert["message"],
                        db.safe_json(alert.get("recommended_actions", {})),
        ...
        """, (
            ...
            float(alert["hazard_probabilities"].get("normal", 0.0)),
            alert["message"],
            json.dumps(alert.get("recommended_actions", {}), default=str),
            alert.get("explanation"),
        ))
                ...
                with db.connect() as conn:
                    if location:
                        rows = conn.execute("""
                            ...
                            WHERE is_active AND location = %s
    ...
    def dismiss_alert(self, alert_id: int) -> None:
        ...
        try:
            ...
            if self.db_path is None and db.is_configured():
                with db.connect() as conn:
                    conn.execute("UPDATE multi_hazard_alerts SET is_active = FALSE WHERE alert_id = %s", (alert_id,))
                    conn.commit()
                return
        except Exception:
            pass

...
def fetch_weather_with_hardening(
    ...
    if live_data is not None:
        ...
        # Stale fallback: always flag as stale because the caller is getting
        # last-known-good data outside the live freshness window.
        response = dict(cached)
        response["stale"] = True
        response["data_source"] = "cached_stale"
        response["api_error"] = api_error
        if "stale_warning" not in response:
            response["stale_warning"] = (
                f"Data may be stale, last updated {response.get('last_updated', utcnow().isoformat())}"
            )
        if "last_updated" not in response:
            response["last_updated"] = utcnow().isoformat()
    else:
